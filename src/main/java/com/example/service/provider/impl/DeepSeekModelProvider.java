...
public class DeepSeekModelProvider implements ModelProvider {
    ...
    public DeepSeekModelProvider(WebClient.Builder webClientBuilder,
    ...
    public void init() {
        ...
        log.info("DeepSeek模型提供者已注册: {}", PROVIDER_NAME);
    }
    ...
    public Flux<SseEventResponse> streamChat(ChatRequest request) {
        log.info("🚀 DeepSeek开始流式聊天，模型: {}", request.getModelName());
     ...
     * 调用DeepSeek API
     */
    private Flux<SseEventResponse> callDeepSeekApi(ChatRequest request) {
