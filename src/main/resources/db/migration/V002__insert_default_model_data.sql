...
-- 8. 添加数据完整性检查注释
COMMENT ON TABLE ai_providers IS 'AI模型提供者配置表，存储如OpenAI、Qwen等提供者的基础信息';
COMMENT ON TABLE ai_models IS 'AI模型配置表，存储每个提供者下具体模型的配置信息';
COMMENT ON TABLE user_model_preferences IS '用户模型偏好表，记录用户对特定模型的选择偏好';
COMMENT ON VIEW v_available_models IS '可用模型视图，展示所有启用的提供者和模型信息';
COMMENT ON VIEW v_user_model_preferences IS '用户模型偏好视图，展示用户选择的模型偏好详情';