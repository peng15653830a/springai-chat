...
@ContextConfiguration(
    classes = {ChatController.class, com.example.exception.GlobalExceptionHandler.class})
public class ChatControllerTest {
  ...
  void testSendMessage_InvalidConversationId() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/chat/conversations/0/messages")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(messageRequest)))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
  ...
  void testGetSseStream() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/api/chat/stream/1")).andExpect(status().isOk());
