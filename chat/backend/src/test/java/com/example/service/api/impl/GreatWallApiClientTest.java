...
class GreatWallApiClientTest {
  ...
  @Mock private GreatWallProperties greatWallProperties;

  private GreatWallChatApi apiClient;

  @BeforeEach
  void setUp() {
    // 不在全局setUp中进行过度mocking，避免UnnecessaryStubbing
    // 各个测试方法有需要时单独mock
  }
  ...
  void testConstructor() {
    ...
    when(webClientBuilder.codecs(any()))
        .thenAnswer(
            invocation -> {
              // 返回this以支持链式调用
    ...
    // 执行测试
    apiClient =
        new GreatWallChatApi(
            webClientBuilder, objectMapper, multiModelProperties, greatWallProperties);
  ...
  void testGetApiEndpoint() {
    ...
    // 验证结果
    assertThat(apiEndpoint).isEqualTo("https://api.greatwall.com");
  }
  ...
  void testIsAvailableWithUnicodeCharactersInProviderName() {
    ...
    // 执行测试
    GreatWallChatApi unicodeApiClient =
        new GreatWallChatApi(
            webClientBuilder, objectMapper, multiModelProperties, greatWallProperties);
  ...
  void shouldExecuteChatCompletionStreamSuccessfully() {
    ...
    when(multiModelProperties.getProviders())
        .thenReturn(java.util.Map.of("greatwall", providerConfig));
    when(multiModelProperties.getApiKey("greatwall")).thenReturn("test-api-key");
    ...
    List<ChatCompletionRequest.ChatMessage> messages =
        List.of(ChatCompletionRequest.ChatMessage.builder().role("user").content("Hello").build());
    ...
    when(responseSpec.bodyToFlux(String.class))
        .thenReturn(
            Flux.just(
                "{\"event\":\"llm_chunk\",\"data\":{\"choices\":[{\"delta\":{\"content\":\"test response\"}}]}}",
                "[DONE]"));
