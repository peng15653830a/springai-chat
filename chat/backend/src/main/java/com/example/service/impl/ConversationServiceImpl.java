      ...
      for (Message msg : messages) {
        try {
          if (msg != null && "assistant".equalsIgnoreCase(msg.getRole())) {
            List<SearchResult> results = messageToolResultService.getMessageSearchResults(msg.getId());
          ...
          log.warn("填充搜索结果时发生异常，messageId: {}，错误: {}", msg != null ? msg.getId() : null, e.getMessage());
        }
      }
    }
