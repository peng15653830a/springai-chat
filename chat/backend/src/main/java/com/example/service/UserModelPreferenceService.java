...
/**
 * 用户模型偏好服务接口 专注于用户模型偏好的CRUD操作，不涉及模型查询逻辑
 *
 * @author xupeng
 */
public interface UserModelPreferenceService {
   ...
   * @return 是否删除成功
   */
  boolean deleteUserModelPreference(
      com.example.dto.request.DeleteUserModelPreferenceRequest request);
  ...
  default boolean deleteUserModelPreference(Long userId, String providerName, String modelName) {
    return deleteUserModelPreference(
        com.example.dto.request.DeleteUserModelPreferenceRequest.builder()
            .userId(userId)
