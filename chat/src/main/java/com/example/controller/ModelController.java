...
public class ModelController {
  ...
  public ApiResponse<ModelInfo> getModelInfo(
      @PathVariable String providerName, @PathVariable String modelName) {
  ...
  public ApiResponse<Boolean> checkModelAvailability(
      @PathVariable String providerName, @PathVariable String modelName) {
  ...
  public ApiResponse<UserModelPreferenceDto> getUserDefaultModel(@PathVariable Long userId) {
    ...
    try {
      ...
      if (preference != null) {
        log.info(
            "用户 {} 的默认模型: {}-{}", userId, preference.getProviderName(), preference.getModelName());
  ...
  public ApiResponse<List<UserModelPreferenceDto>> getUserModelPreferences(
      @PathVariable Long userId) {
    log.info("获取用户 {} 的所有模型偏好", userId);

    try {
      List<UserModelPreferenceDto> preferences =
          userModelPreferenceService.getUserModelPreferences(userId);
  ...
  public ApiResponse<Boolean> saveUserModelPreference(
      @RequestBody com.example.dto.request.UserModelPreferenceRequest request) {
    log.info(
        "保存用户模型偏好，用户ID: {}, 模型: {}-{}, 是否默认: {}",
        request.getUserId(),
  ...
  public ApiResponse<Boolean> deleteUserModelPreference(
      @RequestBody com.example.dto.request.DeleteUserModelPreferenceRequest request) {
    log.info(
        "删除用户模型偏好，用户ID: {}, 模型: {}-{}",
        request.getUserId(),
        request.getProviderName(),
